...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating }) => {
  ...
  useEffect(() => {
    ...
    // Expiry/Feature popup config (Legacy check logic reused but pointing to new config or separated?)
    // The user wants "popup 2.0" which is "ThreeTierPopup".
    // We should use threeTierPopupConfig for enabling/disabling.
    
    // Note: The original logic combined Expiry Popup and Feature Popup logic.
    // We should ensure we are using the correct enabled flag.
    if (!config?.enabled) return;

    const checkPopup = () => {
      const now = Date.now();
      // Use hardcoded interval or add to config if needed, default to 60 mins for now
      ...
      if (isSubscribed && user.subscriptionEndDate) {
          ...
          if (hoursLeft > 0 && hoursLeft <= 48) { // Hardcoded 48h as config doesn't have it anymore
      ...
      if (config.showToPremium && isSubscribed) shouldShowFeature = true;
