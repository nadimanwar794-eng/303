...
import { User, Subject, StudentTab, SystemSettings, CreditPackage, WeeklyTest, Chapter, MCQItem } from '../types';
...
import { Layout, Gift, Sparkles, Megaphone, Lock, BookOpen, AlertCircle, Edit, Settings, Play, Pause, RotateCcw, MessageCircle, Gamepad2, Timer, CreditCard, Send, CheckCircle, Mail, X, Ban, Smartphone, Trophy, ShoppingBag, ArrowRight, Video, Youtube, Home, User as UserIcon, Book, BookOpenText, List, BarChart3, Award, Bell, Headphones, LifeBuoy, WifiOff, Zap, Star, Crown, History, ListChecks } from 'lucide-react';
...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating }) => {
  ...
  const startChallenge = () => {
      const config = settings?.dailyChallengeConfig || { rewardPercentage: 90, mode: 'AUTO', selectedChapterIds: [] };
      ...
      if (questions.length === 0) {
          showAlert("Not enough content to generate a challenge yet. Try browsing some chapters first!", 'ERROR');
          return;
      }
      ...
      const test: WeeklyTest = {
          ...
          description: "Complete 100 questions in 15 minutes to win!",
          isActive: true,
          classLevel: user.classLevel || '10',
          questions: questions,
      ...
      if (onStartWeeklyTest) onStartWeeklyTest(test);
  };
          ...
          return (
              <div className="space-y-6 pb-24">
                  ...
                  <div className="space-y-4 mb-6">
                      {/* Global Syllabus Switch Removed */}
                      ...
                      <button 
                          ...
                          className="mx-1 mt-6 mb-4 bg-gradient-to-r from-violet-600 to-indigo-600 p-4 rounded-2xl shadow-xl shadow-indigo-200 text-white flex items-center justify-between relative overflow-hidden group border border-white/20 active:scale-[0.98] transition-all"
                      >
                          <div className="relative z-10 text-left">
                              <h3 className="text-xl font-black italic flex items-center gap-2">
                                  <Trophy className="fill-yellow-400 text-yellow-400 animate-bounce" /> Daily Challenge
                              </h3>
                              <p className="text-xs font-medium text-indigo-100 mt-1">
                                  Solve 100 Qs & Win <span className="font-bold text-white bg-white/20 px-1 rounded">Free Subscription</span>
                              </p>
                          </div>
                          <div className="relative z-10 bg-white/10 p-3 rounded-full backdrop-blur-sm group-hover:rotate-12 transition-transform">
                          ...
                          <div className="absolute bottom-0 left-0 w-20 h-20 bg-yellow-400/20 rounded-full translate-y-1/2 -translate-x-1/2 blur-xl"></div>
                      </button>
                  ...
                  <div>
                      ...
                      <div className="grid grid-cols-4 gap-3">
                          ...
                          {/* REPLACED CHAT WITH SUPPORT */}
                          <button onClick={() => setShowSupportModal(true)} className="aspect-square bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center gap-1 hover:bg-slate-50 transition-all group">
                              <div className="bg-blue-50 p-2 rounded-full group-hover:bg-blue-100 transition-colors"><Headphones size={20} className="text-blue-500" /></div>
              ...
              <div className="space-y-6 pb-24">
                      ...
                      <div className="bg-purple-50 p-4 rounded-2xl border border-purple-100">
                          <div className="flex justify-between items-center mb-2">
                              ...
                              <button 
                                  onClick={startChallenge}
                                  className="bg-purple-600 text-white text-[10px] font-bold px-3 py-1.5 rounded-full flex items-center gap-1 shadow-lg animate-pulse"
                              >
                                  <Zap size={12} className="fill-white" /> Daily Challenge (Win Sub)
                              </button>
                          </div>
                          <div className="grid grid-cols-2 gap-2">
                              {getSubjectsList(user.classLevel || '10', user.stream || null).map(s => (
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
        ...
        {/* {showChat && <ChatHub user={user} settings={settings} onClose={() => setShowChat(false)} />} */}
